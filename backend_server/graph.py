...
logging.basicConfig(
    ...
    level=logging.INFO,
    force=True,
    format="G1%(asctime)s - %(levelname)s - %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S"
)    
...
appSock = socketio.ASGIApp(
    sio, other_asgi_app=appFAPI
)  # Integrate Socket.IO with FastAPI app
...
async def connect(sid, environ):
    await sio.emit("message_livestatus", "hello")
...
def run_fastapi():
    print("Starting FastAPI server on port 5091...")
    uvicorn.run(appFAPI, host="0.0.0.0", port=5092, log_level="info")


def run_sock():
    print("Starting FastAPI server on port 7771...")
    uvicorn.run(appSock, host="0.0.0.0", port=7771)
...
async def Generator(subQueries, verification=" "):
    global agent_counter  # Declare it as global
    ...
    async def process_subquery(sub_query):
        ...
        await sio.emit(
            f"message_to_frontend_subquery_start_{agent_counter}", json.dumps(sub_query)
        )
        st.write("-" * 50)
        ...
        async for output in app.astream(
            {"question": sub_query, "revision_number": 0, "max_revisions": 2, "first_attempt": True}
        ):
            # Capture output generation steps
            for key, value in output.items():
                st.write(f"#### At Node {key}")
                data = {"sub_query": sub_query, "node": key}  # Add sub_query  # Add key

                await sio.emit(
                    f"message_to_frontend_atnode_{agent_counter}", json.dumps(data)
                )
            ...
            if "final_generation" in value:
                await sio.emit(
                    f"message_to_frontend_subquery_end_{agent_counter}",
                    json.dumps(data),
                    callback=lambda: print("Message sent to frontend by agent 3"),
                )
...
global key_queries, exec_queries, business_queries
...
async def process_question(company , netQueries):
    global agent_counter  # Declare it as global
    global exec_queries, key_queries, business_quries
    global final_report

    await sio.emit(f"send1", "at process question")
    ...
    for queries in netQueries:
        ...
        # here quries is one of key_queries / business_queries / exec queries
        ...
        st.write(f"message_to_frontend_subquery_array_{agent_counter}")

        print("------------------SENDING SOCKET--------------")
        await sio.emit(f"send1", "aedasdfadmin")
        await sio.emit(
            f"message_to_frontend_subquery_array_{agent_counter}",
        ...
        if generations:
            await sio.emit(
                f"message_to_frontend_all_subs_processed_{agent_counter}", "hello"
            )
        ...
        if generations:
            with get_openai_callback() as cb:
                ans = aggregator.invoke(
                    {"question": queries, "answers": ", ".join(answers)}
                )
            ...
            print("### Final Answer:")
            await sio.emit(
                f"message_to_frontend_answer_{agent_counter}",
                json.dumps(ans.answer),
                callback=lambda: print(
                    f"Message sent to frontend by agent {agent_counter}"
                ),
            )
            st.write(ans.answer)
...
#------
    ...
    save_to_pdf(final_report,f"Final_report_gen.pdf")
    await sio.emit(
            "message_to_frontend_final_answer",
            json.dumps(final_report),
            callback=lambda: print(f"Answer sent to frontend by agent"),
        )
    ...
    Discussion(
        company= company, 
        data=data,
        key_agent=key_metrics_agent_mode,
        business_agent=business_model_agent_mode,
        executive_agent=executive_agent_mode,
    )
...
thread_ = threading.Thread(target=run_fastapi)
...
#     asyncio.create_task(runReportGenCode())

# async def send_socket():
#     print("Emit function called")
#     await sio.emit("send1", "inside sendsocket function")

# asyncio.run(send_socket())


# print("---------send sokcet ran")


# @appFAPI.post("/trigger")
# async def trigger_helloworld(request: RequestModel):
#     await runreportGenCode(request.ques)
#     # Add a 30 second wait time to simulate a long-running process
...
#     #     await sio.emit(
#     #         "message_to_frontend_final_answer",
#     #         json.dumps(Generated_Report),
#     #         callback=lambda: print("Answer sent to frontend by agent"),
#     #     )
#     global final_report
#     Generated_Report = final_report
#     with open("report_dashboard.json", 'r') as f:
#         Generated_json = json.load(f)

#         # st.write(Generated_Report)

#         save_to_pdf(Generated_Report)
#         # Generated_json = get_json(Generated_Report)
#         # st.write(Generated_json)
#     return {
#         "message": "helloworld sent to all clients",
#         "report_text": Generated_Report,
#         "report_json": Generated_json,
#     }


# async def send_helloworld():
#     # Emit the event "send1" with data "helloworld" to all connected clients
#     await sio.emit("send1", "helloworld")
#     print('Sent "helloworld" with key "send1" to all clients')


async def runreportGenCode(ques1):
    # Check if the file exists and delete it
    if os.path.exists(filename):
        os.remove(filename)
        print(f"{filename} already exists. It has been deleted.")

    global exec_queries, business_quries, key_queries

    input_queries101 = f"Ask the relevant Questions to the RAG about {ques1} to produce a relavant Financial Due Deligence Report."
    ...
    print("business_queries:", business_quries)
    print("key_queries:", key_queries)

    net_queries = [key_queries, exec_queries, business_quries]

    await process_question(ques1, net_queries)
...
if run_button and query_input:
    ...
    if generations:
        with get_openai_callback() as cb:
            ans = aggregator.invoke(
                {"question": query_input, "answers": ", ".join(answers)}
            )
